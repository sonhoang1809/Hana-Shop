...
public class VerifyEmailController extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try {
            ...
            String confirm = request.getParameter("codeConfirm");
            if (code.equals(confirm)) {
                session.removeAttribute("code");
                url = SUCCESS;
            } else {
                request.setAttribute("ERRORCODECONFIRM", "Your code input is not correct!!");
            }
        } catch (Exception ex) {
            log.error("Error at VerifyEmailController: " + ex.toString());
            SendMailSSL.sendToAdmin("Error at VerifyEmailController: " + ex.toString(), "Error!!");
        } finally {
